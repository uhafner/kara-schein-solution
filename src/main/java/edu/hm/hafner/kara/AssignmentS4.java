...
public class AssignmentS4 {
    ...
    public static void main(final String... unused) {
        ...
        var x0 = askNumber("X-Koordinate linke obere Ecke Rechteck (Spalte 0 - n-1): ");
        var y0 = askNumber("y-Koordinate linke obere Ecke Rechteck (Zeile 0 - n-1): ");
        ...
        if (x0 < size && y0 < size) {
            ...
            walk(y0);
            turnLeft();
            for (int y = y0; y < Math.min(y0 + height, size); y++) {
                walk(x0);
                draw(Math.min(width, size - x0));
                walk(Math.max(0, size - width - x0));
                moveDown();
            }
            for (int y = Math.min(y0 + height, size); y < size; y++) {
                moveDown();
            }
        }
    }
